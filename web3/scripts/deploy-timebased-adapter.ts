...
async function main() {
  console.log("\n🚀 Deploying TimeBased SimpleMockAdapter...\n");

  const routerAddress = "0x8c1Fef12BaFC06077C06486bF4c3E0c9c1F78e78";

  const SimpleMockAdapter = await ethers.getContractFactory("SimpleMockAdapter");
  ...
  console.log("   Annual Rate:", await adapter.ANNUAL_RATE_BP(), "basis points (10% APY)");
  console.log("   Last Harvest:", new Date((Number(await adapter.lastHarvestTime()) * 1000)).toLocaleString());

  console.log("\n⚠️  Next steps:");
  console.log("1. Update .env.local with new adapter address:");
  console.log(`   NEXT_PUBLIC_ADAPTER_ADDRESS=${adapterAddress}`);
  console.log("\n2. Configure router to use new adapter:");
  console.log(`   npx tsx scripts/configure-adapter.ts ${adapterAddress}`);
